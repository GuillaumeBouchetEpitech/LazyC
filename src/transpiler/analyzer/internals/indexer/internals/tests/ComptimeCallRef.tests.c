...
static void ComptimeCallRef_can_be_created_and_freed() {
  ...
  assert(newComptimeCallRef != NULL);
...
static void ComptimeCallRef_can_list_the_comptime_args() {
  ...
  const char* argStr0 = newComptimeCallRef->argsList->data[0];
  assert(strcmp(argStr0, "int") == 0);

  const char* argStr1 = newComptimeCallRef->argsList->data[1];
  assert(strcmp(argStr1, "Vec3*") == 0);

  const char* argStr2 = newComptimeCallRef->argsList->data[2];
