...
export default grammar({
  ...
  conflicts: $ => [
    ...
    [$.struct_specifier], // <- MY STUFF
    // [$.type_specifier, $.comptime_args_list], // <- MY STUFF
    [$.type_specifier, $.comptime_args_list_item], // <- MY STUFF
    [$.expression, $.comptime_call_expression], // <- MY STUFF
    [$.pointer_declarator], // <- MY STUFF
    [$.pointer_type_declarator], // <- MY STUFF
    [$.pointer_field_declarator], // <- MY STUFF
    [$.abstract_pointer_declarator], // <- MY STUFF
    [$.pointer_declarator, $.abstract_pointer_declarator], // <- MY STUFF
    [$.union_specifier] // <- MY STUFF
  ],
  ...
  rules: {
    ...
    _top_level_item: $ => choice(
      $.custom_import, // MY STUFF
      $.impl_specifier, // MY STUFF
      $.function_definition,
    ...
    _block_item: $ => choice(
      // $.exported_function_definition, // <- MY STUFF
      $.function_definition,
      alias($._old_style_function_definition, $.function_definition),
      $.linkage_specification,
      $.declaration,
    ...
    impl_specifier: $ => prec.right(PREC.IMPL, seq(
      "impl",
      field('name', $._type_identifier),
      optional(seq("<", field('templated_type', $._type_identifier), ">")), // <- MY STUFF
      '{',
      field('body', repeat(
        choice(
          // $.exported_function_definition, // <- MY STUFF
    ...
    // // MY STUFF
    // exported_function_definition: $ => seq(
    //   "export",
    //   $.function_definition
    // ),

    function_definition: $ => seq(
      optional("export"), // <- MY STUFF
      optional(choice("comptime", "test")), // <- MY STUFF
    ...
    type_specifier: $ => choice(
      ...
      $.primitive_type,
      $._type_identifier,
      $.comptime_call_expression, // <- MY STUFF
    ),
    ...
    struct_specifier: $ => prec.right(seq(
      ...
      choice(
        seq(
          field('name', $._type_identifier),
          optional(field('templated_type', seq("<", $._type_identifier, ">"))), // <- MY STUFF
    ...
    _expression_not_binary: $ => choice(
      ...
      $.call_expression,
      $.comptime_call_expression, // <- MY STUFF
      $.static_call_expression, // <- MY STUFF
      $.field_expression,
    ...
    // MY STUFF
    struct_ctor_call: $ => seq(
      field('type', $.identifier),
      field('body', seq("{", optional(seq($.struct_ctor_call_parameter, repeat(seq(",", $.struct_ctor_call_parameter)))), optional(","), "}"))
    ),
    struct_ctor_call_parameter: $ => seq(field('name', $.identifier), ":", field('value', $.expression)),
    // MY STUFF

    // MY STUFF
    custom_import: $ => seq(
      "import",
      seq("{", field('body', seq($.identifier, repeat(seq(",", $.identifier)))), optional(","), "}"),
      "from",
      field('path', $.string_literal),
    ),
    // custom_import_parameter: $ => seq(field('name', $.identifier), ":", field('value', $.expression)),
    // MY STUFF

    _string: $ => prec.left(choice(
      $.string_literal,
      $.concatenated_string,
      $.super_string_literal, // <- MY STUFF
    )),
    ...
    // MY STUFF
    comptime_call_expression: $ => seq(
      field('function', $.identifier),
      field('arguments', $.comptime_args_list),
    ),
    comptime_args_list: $ => seq('<', commaSep($.comptime_args_list_item), '>'),
    comptime_args_list_item: $ => seq(choice($.identifier, $.string_literal, $.type_specifier), repeat('*')),
    // /MY STUFF

    // MY STUFF
    static_call_expression: $ => prec(PREC.CALL, seq(
      field('namespace', choice($._statement_identifier, $.comptime_call_expression)),
      "::",
      $.call_expression,
    )),
    // /MY STUFF
    ...
    // MY STUFF
    super_string_literal: $ => seq(
      "```",
      repeat(choice(
        // alias(token.immediate(prec(1, /[^\\"\n]+/)), $.super_string_content),
        // $.escape_sequence,
        alias(token.immediate(prec(1, /[^\`\`\`]+/)), $.super_string_content),
      )),
      "```",
    ),
    ...
    // comptime_comment: _ => token(
    //   seq('///comptime-lazy-c:', /(\\+(.|\r?\n)|[^\\\n])*/),
    //   // seq("///comptime-lazy-c:", /((.|\r?\n)|[^\\\n])*/),
    // ),
